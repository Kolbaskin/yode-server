...
Ext.define('MyDesktop.modules.users.controller.Groups', {
    ...
    ,onCheckColumnsClick:function( ct, column, e, t, eOpts ) {
        ...
        var selAll = true, store = ct.ownerCt.getStore(), data = [];
    ...
    ,saveModelAccess: function(form, data) {
        ...
        stor.each(function(r) {
            if(r.data.read || r.data.add || r.data.modify || r.data.del)
    ...
    ,modulesStartEdit: function(form, data) {
        var me = this, store = form.down('[action=model-access]').getStore()
        store.removeAll();        
    ...
    ,getModels: function(callback) {
        ...
        Core.Ajax.request({
            ...
            succ: function(data) {
                me.modelsAll = []//data.list
                for(var i=0;i<data.list.length;i++) {
                    me.modelsAll.push({name: data.list[i].name, hname: D.t(data.list[i].name)})
                }
                callback(me.modelsAll)
            }                     
        })   
